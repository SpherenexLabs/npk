 ...
 * - Hardened against Recharts empty-data crash with guarded rendering and animations disabled.
 */
...
function knnSuggest(sample, k = 3) {
    ...
    for (const n of neighbors) counts.set(n.row.label, (counts.get(n.row.label) || 0) + 1);
    ...
    const reasons = neighbors.filter((n) => n.row.label === majorityLabel).map((n) => n.row.suggestion);
    const suggestionText =
        reasons[0] || neighbors[0]?.row?.suggestion || "No suggestion available. Consider adding more training samples.";
...
/* Guard: render a friendly placeholder until we have some data */
const HasData = ({ data, height = 300, children }) => {
...
const WaterDetectionDashboard = () => {
    ...
    useEffect(() => {
        ...
        return () => {
            // Only call the returned unsubscribers; no need to also call off()
            infoUnsub();
            dataUnsub();
        };
    }, [db]);
                ...
                <div style={styles.card}>
                    ...
                    <div style={styles.parameterGrid}>
                        <div style={styles.parameter}>
                            ...
                            <span style={{ ...styles.paramValue, color: getStatusColor(temp, "temp") }}>{temp}°C</span>
                        </div>
                        <div style={styles.parameter}>
            ...
            <div style={styles.chartsSection}>
                ...
                {showCharts.waterQuality && (
                    <div style={styles.chartCard}>
                        ...
                        <HasData data={historicalData} height={300}>
                            <ResponsiveContainer width="100%" height={300}>
                                <LineChart data={historicalData}>
                                    ...
                                    <Line isAnimationActive={false} yAxisId="ph" type="monotone" dataKey="ph" stroke="#e53e3e" strokeWidth={2} name="pH" />
                                    <Line isAnimationActive={false} yAxisId="tds" type="monotone" dataKey="tds_ppm" stroke="#3182ce" strokeWidth={2} name="TDS (ppm)" />
                                    <Line isAnimationActive={false} yAxisId="tds" type="monotone" dataKey="ec" stroke="#805ad5" strokeWidth={2} name="EC" />
                                </LineChart>
                            </ResponsiveContainer>
                        </HasData>
                    </div>
                ...
                {/* NPK Chart */}
                {/* NPK Chart — switched to LineChart */}
                {showCharts.npk && (
                    <div style={styles.chartCard}>
                        ...
                        <HasData data={historicalData} height={300}>
                            <ResponsiveContainer width="100%" height={300}>
                                <LineChart data={historicalData}>
                                    ...
                                    <Tooltip content={<CustomTooltip />} />
                                    <Legend />
                                    <Line isAnimationActive={false} type="monotone" dataKey="n" stroke="#38a169" strokeWidth={2} name="Nitrogen (N)" dot={false} />
                                    <Line isAnimationActive={false} type="monotone" dataKey="p" stroke="#d69e2e" strokeWidth={2} name="Phosphorus (P)" dot={false} />
                                    <Line isAnimationActive={false} type="monotone" dataKey="k" stroke="#e53e3e" strokeWidth={2} name="Potassium (K)" dot={false} />
                                </LineChart>
                            </ResponsiveContainer>
                        </HasData>
                    </div>
                        ...
                        <HasData data={historicalData} height={300}>
                            <ResponsiveContainer width="100%" height={300}>
                                <LineChart data={historicalData}>
                                    ...
                                    <Line isAnimationActive={false} yAxisId="temp" type="monotone" dataKey="temp" stroke="#e53e3e" strokeWidth={2} name="Temperature (°C)" />
                                    <Line isAnimationActive={false} yAxisId="hum" type="monotone" dataKey="hum" stroke="#3182ce" strokeWidth={2} name="Humidity (%)" />
                                    <Line isAnimationActive={false} yAxisId="temp" type="monotone" dataKey="turbidity_ntu" stroke="#805ad5" strokeWidth={2} name="Turbidity (NTU)" />
                                </LineChart>
                            </ResponsiveContainer>
                        </HasData>
                    </div>
                ...
                {/* Water Level Chart */}
                {/* Water Level Chart — switched to LineChart */}
                        ...
                        <HasData data={historicalData} height={300}>
                            <ResponsiveContainer width="100%" height={300}>
                                <LineChart data={historicalData}>
                                    ...
                                    <Line isAnimationActive={false} type="monotone" dataKey="water_pct" stroke="#3182ce" strokeWidth={2} name="Water Level (%)" dot={false} />
                                </LineChart>
                            </ResponsiveContainer>
                        </HasData>
                    </div>
            ...
            <div style={styles.fullWidthCard}>
                ...
                <div style={{ display: "grid", gap: 14 }}>
                    ...
                    <div style={styles.metaChips}>
                        ...
                        <span style={styles.outlineChip}>Neighbors: {Math.max(0, ai.neighbors?.length || 0)}</span>
                        <span style={styles.outlineChip}>Data Points: {historicalData.length}</span>
                    </div>

                    <button onClick={() => setShowNN((v) => !v)} style={styles.outlinedBtn}>
